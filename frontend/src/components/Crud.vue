                    ...
                    <form @submit.prevent="true">
                        <div v-if="current" class="grid grid-cols-6 gap-6">
                            <div v-for="(field,index) in editFields" :key="index" :class="field.double?'col-span-6':'col-span-3'">
                                ...
                                <Multiselect v-else-if="field.type == 'multiselect' && field.options" :id="'edit-'+field.index" :ref="'edit:'+field.index" v-model="current[field.index]" mode="tags" :create-tag="true" :append-new-tag="true" :searchable="true" :options="field.options" />
                                <SingleSelect v-else-if="field.type == 'select' && field.options" :id="'edit-'+field.index" :ref="'edit:'+field.index" v-model="current[field.index]" :field="field" :options="field.options" @input="change" />
                                <Multiselect v-else-if="field.type == 'multiselect' && !field.options" :id="'edit-'+field.index" :ref="'edit:'+field.index" v-model="current[field.index]" mode="tags" :create-tag="true" :append-new-tag="true" :searchable="true" :options="current[field.index]" />
                                <SingleSelect v-else-if="field.type == 'select' && !field.options" :id="'edit-'+field.index" :ref="'edit:'+field.index" v-model="current[field.index]" :field="field" :options="current[field.index]" @input="change" />
                                <div v-else>
